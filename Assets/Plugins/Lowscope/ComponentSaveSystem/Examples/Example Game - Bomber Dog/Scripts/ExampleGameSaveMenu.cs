...
namespace Lowscope.Saving.Examples
{
    ...
    public class ExampleGameSaveMenu : MonoBehaviour
    {
        ...
        private void Awake()
        {
            SaveMaster.OnLoadingFromDiskCorrupt += OnSlotCorrupted;
        ...
        private void OnSlotCorrupted(int obj)
        {
            ReloadSlots(mode);
            Debug.Log("Slot corrupted. In case you want to automatically remove corrupted files, there is an option in settings. " +
                      "you can use SaveMaster.OnLoadingFromDiskCorrupt callback to perform actions such as these to let the user " +
                      "know saving has failed.");
        }
