...
namespace Lowscope.Saving
{
    ...
    public class SaveMaster : MonoBehaviour
    {
        ...
        internal static string GetSceneName(int sceneInstanceId)
        {
            ...
            Debug.LogError("Scene name not found? Have you renamed a scene with saveables in it?" + sceneInstanceId);
            return "";
        }
        ...
        */
        ...
        public static void SetSlotWithoutSave(int slot)
        {
            ...
            OnLoadingFromDiskBegin(slot);
            
            activeSaveGame = SaveFileUtility.LoadSave(slot, true, () =>
            {
                OnLoadingFromDiskCorrupt(slot);
            }, SaveSettings.Get().createNewSaveFileOnFullCorruption);
            
            OnLoadingFromDiskDone(slot);
        ...
        public static void SetSlot(int slot, bool reloadSaveables, SaveGame saveGame = null, bool keepActiveSaveData = false, bool writeToDiskAfterChange = false)
        {
            ...
            {
                Debug.LogWarning(string.Format("SaveMaster: Attempted to set illegal slot: {0}", slot));
                return;
            }
        ...
        public static DateTime GetSaveCreationTime(int slot)
        {
            ...
            return getSave != null? getSave.creationDate : default(DateTime);
        }
        ...
        public static TimeSpan GetSaveTimePlayed(int slot)
        {
            ...
            SaveGame getSave = GetSave(slot, true);
            return getSave != null? getSave.timePlayed : default(TimeSpan);
        }
        ...
        /// <summary>
        /// Writes active slot to another slot
        /// </summary>
        /// <param name="slot"></param>
        public static void WriteToOtherSlot(int slot, bool syncSaveables = true)
        {
            if (activeSaveGame == null)
            {
                Debug.Log("Tried to save without an active slot set");
                return;
            }
            ...
            {
                OnWritingToDiskBegin.Invoke(slot);
            ...
            SetMetaDataMultiple(metaData =>
            {
                ...
                metaData.SetData("version", activeSaveGame.gameVersion.ToString());
                metaData.SetData("creationdate", activeSaveGame.creationDate.ToString());
                metaData.SetData("lastsavedtime", DateTime.Now.ToString());
            }, slot);
                
            OnWritingToDiskDone.Invoke(slot);
            invokedWritingToDiskEvent = false;
        ...
        /// <summary>
        /// Loads data from other slot to active slot.
        /// </summary>
        public static void LoadFromOtherSlot(int otherSlot, bool reloadSaveables = true)
        {
            if (activeSaveGame == null)
            {
                Debug.Log("Tried to load without an active slot set");
                return;
            }
        ...
        public static void ClearListeners(bool syncSave, Scene scene)
        {
            ...
            List<Saveable> markedForRemoval = new List<Saveable>();
            bool shouldSave = syncSave && activeSaveGame != null;
            
            foreach (var saveable in saveables)
            {
                if (saveable.gameObject.scene != scene) 
                    continue;
        ...
        public static void RemoveListener(Saveable saveable)
        {
            ...
            // But only if auto save on exit is turned on.
            if (!isQuittingGame || !AutoSaveOnExit) 
                return;
        ...
        /// <summary>
        /// Allows you to set multiple metadata through one IO write.
        /// </summary>
        /// <param name="metaData">Action that returns metadata file.</param>
        /// <param name="slot">File slot</param>
        /// <param name="fileName">File name</param>
        public static void SetMetaDataMultiple(Action<MetaDataFileUtility.MetaData> metaData, 
            int slot = -1, string fileName = "")
        {
            string saveFileName = GetSaveFileName(slot, fileName);
        ...
        public static Action<int> OnLoadingFromDiskCorrupt
        {
            get { return instance.onLoadingFromDiskCorrupt; }
            set { instance.onLoadingFromDiskCorrupt = value; }
        }
        ...
        private Action<int> onLoadingFromDiskCorrupt = delegate { };
