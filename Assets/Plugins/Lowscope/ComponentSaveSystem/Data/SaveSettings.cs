...
namespace Lowscope.Saving.Data
{
    public class SaveSettings : ScriptableObject
    {
        ...
        public static SaveSettings Get()
        {
            ...
            if (savePluginSettings == null)
            {
                Debug.LogWarning("Could not find SavePluginsSettings in resource folder," +
                                 " did you remove it? Using default settings.");
        ...
        [Tooltip("This plugin automatically saves to a A and B archive, based on what is oldest. " +
                 "To ensure a working copy always exists.")]
        public string backupExtensionName = ".b";
        [Tooltip("If both the save and backup are not loadable, should the save be archived? Resulting in slot removal.")]
        public bool archiveSaveFileOnFullCorruption;
        [Tooltip("If both the save and backup are not loadable, should we load a new empty save? Happens after " +
                 "archival if option above is selected.")]
        public bool createNewSaveFileOnFullCorruption;
        ...
        [Tooltip("Time interval in seconds before the auto save happens"), Range(1, 3000)]
        ...
        public class SaveablePrefabThrottleOptions
        {
            ...
            [Range(0,120), Tooltip("Will wait a frame when the framerate of the game is lower then specified amount")]
            public int throttleTargetFramerate = 30;
            [Range(0,200), Tooltip("Will wait a frame when the total milliseconds of spawning objects adds upto 30 ms")]
            public int throttleTargetMilliseconds = 30;
            ...
            public bool disableAutoSaveDuringThrottle = true;
