...
namespace Lowscope.Saving.Components
{
    ...
    public class SaveScreenshot : MonoBehaviour
    {
        ...
        public void StoreScreenShot(int slot)
        {
            ...
            // Extensive amount of checks to ensure camera is actually there.
            if (camera == null || camera.Equals(null) || (object)camera == null)
            {
                Debug.Log("Failed to create screenshot: No camera reference found");
                return;
            }
            ...
            SaveMaster.SetMetaDataMultiple(metaData =>
            {
                metaData.SetData("screenshot-width", resWidth.ToString());
                metaData.SetData("screenshot-height", resHeight.ToString());
                metaData.SetData("screenshot", screenShot);
            });
        }
    }
}