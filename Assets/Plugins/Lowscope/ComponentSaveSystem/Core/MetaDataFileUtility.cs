	...
	public class MetaDataFileUtility
	{
		...
		public class MetaData : IDisposable
		{
			...
			public MetaData(string filePath)
			{
				...
				string extensionName = SaveSettings.Get().backupExtensionName;

				string altPath = FileUtilities.GetAlternativeFilePath(filePath, extensionName);
				string newestFilePath = FileUtilities.GetNewestFilePath(filePath, altPath);
				string otherPath = newestFilePath == filePath ? altPath : filePath;
				bool secondSaveExists = File.Exists(otherPath);

				// No meta data files sound
				...
				if (bothSavesCorrupt)
				{
					...
					FileUtilities.ArchiveFileAsCorrupted(newestFilePath);
			...
			public void Dispose()
			{
				string extensionName = SaveSettings.Get().backupExtensionName;
				string altPath = FileUtilities.GetAlternativeFilePath(filePath, extensionName);
				string savePath = FileUtilities.GetOldestFilePath(filePath, altPath);
